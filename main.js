...
function applyWSLGhostMode(mainWindow) {
  try {
    ...
    try {
      ...
      // Criar um script adicional para aplicar técnicas específicas de anti-captura WSL
      const fs = require('fs');
      const path = require('path');
      const wslScriptPath = path.join(__dirname, 'wsl-ghost-helper.sh');

      // Cria o script se não existir
      if (!fs.existsSync(wslScriptPath)) {
...
if [ -n "$WINDOW_ID" ]; then
  ...
  echo "Não foi possível encontrar a janela do Electron"
        ...
        fs.writeFileSync(wslScriptPath, wslScript);
        fs.chmodSync(wslScriptPath, '755');

        // Executa o script
        const { exec } = require('child_process');
        exec(`bash ${wslScriptPath}`, (error, stdout) => {
          if (error) {
            console.error('Erro ao executar script WSL helper:', error);
          } else {
            console.log('Script WSL helper executado:', stdout);
          }
        });
      }
